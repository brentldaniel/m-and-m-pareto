...
[*One might argue that these are not, technically, Pareto Charts, owing to the missing cumulative frequency line oft found on them. They may be simply termed as a "decreasing frequency bar chart". But I'll call them Pareto Charts here.]
...
This *Dot Plot* is not very satisfying. Notice how each colum is labeled?  Maybe we can adjust the sizes of the circles and decrease the gaps, but that might be a lot of work. We can, in fact, remove those labels, as I will show in the next kind of chart of the same data:
...
##### If "Bag" is the unit of analysis

As I analyzed this data, however, I found that I wanted to know things about the **bags** more often than I wanted to find the attributes of the set as a whole, as if the bags weren't dumped all together.  For example, I wanted to add the **total number of M&Ms** to each bag:
...
*M&Ms come in different sizes. There's a pretty tpyical size (which I call **Regular**), and there's the size you most often see around Halloween (a small bag callled **Fun Size** -- I like to joke that it would be more fun if it were bigger).*
...
If we seperate them out, we can better define the size-ranges:
...
[Formatting note: Have you noticed at this point that the background color on these charts is changing? There's a reason! The light blue ("azure") is the general background, when Kind of M&M is various. The light tan color ("antiquewhite2") represents the Plain M&Ms, because those M&M bags are brown, and the light yellow ("cornsilk") is for the yellow-bagged, Peanut M&Ms!]
...
It's also the case that, though there is a some variation, it is not nearly as wide as the maxmimum. The minimums range from 0% to a bit of 10%.
...
By faceting on Color, and overlaying a Density curve we also see how Red, Brown and Yellow often have smaller numbers within a bag (given how much tighter the distributions are). But we also can see that Yellow, Organge and Blue have flatter, more even distributions. I think, a naive viewer of M&Ms would guess that all colors would have similar distributions.

This may be a good time for a statistical test. Is the mean for Blue different than the mean for Red?

```{r}
#test Plain, Blue vs. Red
...
Our eyes do not deceive us! Yes, that's statistically different. Even when dividing between Plain and Peanut.
...
*For a Regular-sized, Plain bag of M&Ms, it's pretty unusual! However, for bags with smaller counts, it can happen pretty regularly. I've yet to run the probability calculations to see if the rate is higher or lower than it "should be"*

* how does a bag have only five of the six colors?

*This is equally unusual for Regular, Plain M&Ms, but again, with smaller-count bags (Fun Size, or Peanut Regular Size), it can happen more often*

* does blue really dominate the counts, as it seems to?

*Blue did show up more frequently than any other color, and it was significantly more than the least frequent color (Red)*
...
*Yes, and that was one of the more surprising findings (the Mosaic chart)*
...
Last point I want to make: **more data?**  Would you consider adding to the data set? All you have to do is:
